...
- [Nougat: Neural Optical Understanding for Academic Documents](https://arxiv.org/abs/2308.13418) - academic paper indroducing Nougat.
...
3. Usage
    ...
    Nougat can be ran on the worker/fcfs nodes:
    ```bash
...
1. Read in a .mmd file and break it into chunks using '#' as delimiter - each chunk will be a subsection of Binney and Tremaine,
2. Read in subsection titles ftom the uncleaned version of the file because cleaning removes line breaks,
3. Select an embedding model and use Ollama API to embed the chunks:
    ```python
    ...
    ```
    See the next section for instructions how to set up Tentence Transformers.
4. The produced NumPy array has hundreds of dimensions - reduce its dimensionality to 2D and 3D using UMAP:
    ...
    ```
    The `metric` parameter controls how distance is computed in the ambient space of the input data. We use the cosine metric that uses the cosine of the angle between two vectors, which reflets the semantic similarity of the two chunks and is commonly used for calculating embeddings. 

5. Visualise and save the embeddings - with `matplotlib` and `bokeh` in 2D, with `plotly` in 3D. 
...
We selected `nomic-embed-text-v1` due to its large context window necessary to embed entire subsections of Binney and Tremaine at once.
